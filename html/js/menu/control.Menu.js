...
function MenuController( $scope, $rootScope )
{
	...
	// For open a link in steam : ) 
	// If you want support our server u can join our discord just bellow :) 
	$scope.OpenDiscord = function () {
		lua.Run("gui.OpenURL('https://discord.gg/tsowari')");
	  };
	  $scope.OpenBoutique = function () {
		lua.Run("gui.OpenURL('https://ts-community.fr/')");
	  };
	  $scope.OpenCollection = function () {
		lua.Run("gui.OpenURL('https://example.com')");
	  };
